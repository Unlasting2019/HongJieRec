...
* 最后将算好的特征以TFRecord格式序列化到磁盘，不定长特征在存的时候不pad到定长，解析的时候用tf.io.VarLenFeature。训练时根据是否需要可以转Dense或者直接reduce，DIN/DIEN/Transformer等模型就需要转Dense，当前doc和历史clk_doc笛卡尔积这个特征就不需要转dense可以使用tf.gather & tf.math.segment_sum完成reduce
