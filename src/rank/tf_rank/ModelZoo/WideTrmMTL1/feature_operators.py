...
class FeatureOp:
    ...
    def SparseMultiNumericOp(self, field, norm=None):
        print(color_print.format(f"{field} - SparseMultiNumericOp"))
        sp_val = tf.expand_dims(self.features[field].values, -1)
        segment_ids = self.features[field].indices[:, 0]
        if norm == 'abs':
    ...
    def SparseSingleOp(self, field, vocab_size):
        ...
        tf.summary.histogram("{field}_vec", x_vec)
        print(f'- x_vec:{x_vec}')
        print(f'- x_emb:{x_weight}')
        return {field:x_vec}

    def SparseNumericOp(self, field, norm=None):
        print(color_print.format(f"{field} - SparseNumericOp"))
        x = self.features[field]
        if norm == 'abs':
    ...
    def DenseMultiNumericOp(self, field, emb_size, norm=None):
        ...
        if norm == 'VLN':
            x_vec /= (tf.math.reduce_std(x_vec, axis=-1, keepdims=True)+1e-12)
        elif norm == 'LN':
            x_vec = tf.keras.layers.LayerNormalization()(x_vec)
    ...
    def DenseNumericOp(self, field, emb_size, norm=None):
    ...
    def DenseSingleOp(self, vocab_size, field, emb_size):
    ...
    def DenseSequenceOp(self, seq_doc, target_doc, seq_attr, unit_fcs):
        ...
        with tf.variable_scope(f"SequenceOp__TransformerPosEmb"):
        ...
        #### Transformer Layer ####
        with tf.variable_scope(f"SequenceOp__TransformerLayer"):
        ...
        with tf.variable_scope(f"SequenceOp__TargetAttentionLayer"):
