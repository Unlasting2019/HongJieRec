...
model_config = {
    ...
    "mlp_layer":{
        "mlp_cf":[
            [64,  lambda x, y : tf_layer.dice(x, y, "mlp_dice2"), 0.],
        ],
        "use_bn":False,
        "use_ln":True,
        "use_residue":False
...
deep_schema = [
    ...
    {'DenseSingleOp':{'vocab_size':347466, 'field':'doc_id', 'emb_size':40}},
    ...
    {'DenseNumericOp':{'field':'user_cate1_exp_cnt', 'norm':'log10', 'emb_size':4}},
    {'DenseNumericOp':{'field':'user_cate1_clk_cnt', 'norm':'log10', 'emb_size':4}},
    ...
    {'DenseNumericOp':{'field':'user_cate2_exp_cnt', 'norm':'log10', 'emb_size':4}},
    {'DenseNumericOp':{'field':'user_cate2_clk_cnt', 'norm':'log10', 'emb_size':4}},
    ...
    {'DenseMultiNumericOp':{'field':'user_title_exp_cnt', 'norm':'log10', 'emb_size':4}},
    {'DenseMultiNumericOp':{'field':'user_title_clk_cnt', 'norm':'log10', 'emb_size':4}},
...
def get_train_ops(model_loss):
    ...
    """ deep embedding """
    deep_sparse_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "deep_part/embedding_part")
    ...
    for var, grad in zip(deep_sparse_vars, deep_sparse_grads):
        print('var:{}\tgrad:{}'.format(var, grad))
        tf.summary.histogram(f"emb_grad/{var.name.replace(':','_')}", grad)
    """ deep mlp """
    deep_dense_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "deep_part/mlp_part")
...
def model_fn(features, labels, mode, params):
    ...
    print(f"linear_schema:{len(linear_schema)}")
    f_op = feature_op.FeatureOp(features, mode, model_config['sparse_reg'], model_config['dense_reg'])

    with tf.variable_scope("linear_part"):
        with tf.variable_scope("is_click"):
            click_inputs = reduce(lambda x, y : {**x, **y}, [f_op.lookup_fn(**conf) for conf in linear_schema])
            click_bias = tf.get_variable(shape=[1], initializer=tf.zeros_initializer, name='click_bias')
            linear_click_logits = tf.add_n(list(click_inputs.values())) + click_bias
    ...
    with tf.variable_scope("deep_part"):
        with tf.variable_scope("embedding_part", reuse=tf.AUTO_REUSE):
            expert_inp = reduce(lambda x, y : {**x, **y}, [f_op.lookup_fn(**cf) for cf in deep_schema])

        with tf.variable_scope("mlp_part"):
            with tf.variable_scope("ple_layer"):
                mmoe_oup = tf_layer.ple_layer(
                    tf.concat(list(expert_inp.values()), -1), mode,
                    **model_config["ple_layer"],
                )
            with tf.variable_scope("click_tower"):
                deep_click_logits = tf_layer.mlp_layer(
                    mmoe_oup["is_click"], mode, 
                    **model_config["mlp_layer"])

            with tf.variable_scope("watch_tower"):
                deep_watch_logits = tf_layer.mlp_layer(
                    mmoe_oup["watch_time"], mode, 
                    **model_config["mlp_layer"])

            click_logit = deep_click_logits + linear_click_logits
            watch_logit = deep_watch_logits
