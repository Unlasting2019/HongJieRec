...
class VecAttGRUCell(RNNCell):
  ...
  def call(self, inputs, state, att_score=None):
    with tf.variable_scope("g_var"):
        ...
        gate = math_ops.sigmoid(gate_([inputs, state]))
        r, z = tf.split(value=gate, num_or_size_splits=2, axis=1)

    with tf.variable_scope("c_var"):
        r_state = r * state 
        c_state_ = _Linear(
            [inputs, r_state],
            self._num_units,
        ...
        c_state = self._activation(c_state_([inputs, r_state]))

    z = (1.0 - att_score) * z
    new_h = z * state + (1 - z) * c_state

    return new_h, new_h


class VecAttLSTMCell(RNNCell):
    pass
